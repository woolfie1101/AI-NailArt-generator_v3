...
export async function extractDominantColors(image: ImageData): Promise<string[]> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.warn(`Color extraction attempt ${attempt}/${maxRetries} failed:`, error.message);

      // If it's a 503 or rate limit, wait before retrying
      if (error.status === 503 || error.status === 429 || error.message?.includes('unavailable')) {
      ...
      if (attempt === 1 && error.status !== 503 && error.status !== 429) {
        break;
      }
    }
  }

  console.error("Error extracting colors with Gemini after all retries:", lastError);
  return [];
}
...
export async function generateTagsForImage(image: ImageData, language: 'en' | 'ko'): Promise<string[]> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      console.warn(`Tag generation attempt ${attempt}/${maxRetries} failed:`, error.message);
  ...
  console.error("Error generating tags with Gemini after all retries:", lastError);
  return [];
}
